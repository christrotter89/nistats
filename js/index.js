...
.controller('dataset', function($scope, tableRenderService, obtainDataService, $location, $route, $routeParams, urlChange, menuService){
	...
	$scope.downloadXLSX = function(JSON, datasetName){
	    alasql('SELECT * INTO XLSX("' + datasetName + '-nistats-download.xlsx",{headers:true}) FROM ?',[JSON]); 
	};

	$scope.downloadCSV = function(JSON, datasetName){
	    alasql('SELECT * INTO CSV("' + datasetName + '-nistats-download.csv",{headers:true}) FROM ?',[JSON]);
	};

	$scope.downloadJSON = function(JSON, datasetName){
	    alasql('SELECT * INTO JSON("' + datasetName + '-nistats-download.json",{headers:true}) FROM ?',[JSON]);
	};
...
.controller('integration', function($scope, menuService, $routeParams){
	...
	$scope.integrationTable = [
		{ 
		 ...
		 {
			key: "meta.category",
			description: "Category classification of dataset. This is not an official designation and used only for classification on NI Stats.",
			dataType: "String"
		 },
		 {
		 ...
		 {
		 	key: "options",
			description: "Contains scales and filters references used for graphing the data on NI Stats.",
